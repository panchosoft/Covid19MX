...
export default {
  ...
  methods: {
    ...
    loadMapAndChart() {
      ...
      for (var i2 = mapData.length - 1; i2 >= 0; i2--) {
        if (mapData[i2].confirmed == 0) {
          mapData.splice(i2, 1);
        }
      }
  ...
  dateAxis.renderer.labels.template.adapter.add("fillOpacity", function(fillOpacity, target){
      ...
      seriesTypeSwitch.events.on("toggled", function() {
        if (seriesTypeSwitch.isActive) {
          ...
          for (var key1 in series) {
            series[key1].hiddenInLegend = true;
            series[key1].hide();
          }

          columnSeries[currentType].show();
        } else {
          for (var key2 in columnSeries) {
            columnSeries[key2].hiddenInLegend = true;
            columnSeries[key2].hide();
          }

          for (var key3 in series) {
            series[key3].hiddenInLegend = false;
            series[key3].hide();
          }
      ...
      function changeDataType(name) {
        ...
        if (seriesTypeSwitch.isActive) {
          ...
          currentSeries.show();
          // hide other series
          for (var key5 in series) {
            if (series[key5] != currentSeries) {
              series[key5].hide();
            }
          }
        }
